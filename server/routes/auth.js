...
router.post('/opret', async (req, res) => {
  try {
    const { navn, kaldenavn, email, kode, gentag_kode, aargang, kollegie, kollegie_andet, myndigheder, note } = req.body;
...
/**
 * GET /api/auth/roller
 * Hent aktive roller (inkl. systemroller)
 */
router.get('/roller', async (req, res) => {
  try {
    const dbRoller = await prisma.rolle.findMany({
      where: { slettet: false },
      orderBy: { navn: 'asc' }
    });

    res.json(['Admin', 'Owner', ...dbRoller.map(r => r.navn)]);
  } catch (error) {
    console.error('Get roller auth error:', error);
    res.status(500).json({ fejl: 'Server fejl' });
  }
});
...
router.put('/profil', requireAuth, async (req, res) => {
  try {
    const { navn, kaldenavn, email } = req.body;
